...
# Add the current directory to path to import solve_redu_inter_four_h
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from solve_redu_inter_four_h import solve_redu_inter_four_h
...
def test_pde_2d_periodic_schemes_h(gamma, M):
    ...
    if single_test:
        ...
        c_int, _, _ = solve_redu_inter_four_h(gamma, h, None, False)
        ...
        for i in range(n_tests):
            ...
            c_int, _, _ = solve_redu_inter_four_frequency_h(gamma, h, None, False)
...
def solve_single_pde_2d_periodic(gamma, M, c_int):
    """
    Solve single 2D PDE with periodic boundary conditions.
    ...
    if use_S_form:
        ...
        coeff = 1 - 2 * np.abs(2 * np.pi * k_wave) ** gamma
        f_2D = coeff * np.sin(2 * np.pi * X) * np.sin(2 * np.pi * Y)
        f_vec = f_2D.flatten()
...
def build_row_internal_periodic(M, h, gamma, coeffs, j):
    ...
    Input:
        ...
        h: grid spacing
        gamma: fractional derivative order
        coeffs: [a1, a2, a3, alpha, beta] from solve_redu_inter_four_h
