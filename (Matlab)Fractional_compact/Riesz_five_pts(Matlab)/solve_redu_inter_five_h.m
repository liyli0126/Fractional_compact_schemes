function [x_opt, A, b] = solve_redu_inter_five_h(gamma, h, x_points_inter,do_plot)
% Input:
...
%  b: right-hand side vector
    ...
    if isscalar(h) && h > 0 && h <= 1
        ...
        if w_max > pi/32
            ...
            x_points = [
                max(w_max / 16, pi/128);   % Low frequency (but not too small)
                w_max / 8;                 % low frequency
                w_max / 4;                 % Mid-low frequency
                w_max / 2;                 % Mid frequency
                min(w_max, 3*pi /4)        % High frequency (capped at 3*pi/4 for stability)
            ];
        else
            % For very small h, use fixed small points
            x_points = [pi/128, pi/64, pi/32, pi/16, pi/8];
        end
        ...
        if length(x_points) < 5
            % Add more points if needed
            additional_points = linspace(pi/128, min(w_max*2, pi/4), 5);
            x_points = sort(unique([x_points(:); additional_points(:)]));
            x_points = x_points(1:min(5, length(x_points)));  % Take first 5
        end
    ...
    elseif isvector(h) && ~isscalar(h)
        ...
        if isempty(x_points) || length(x_points) < 5
            error('x_points_inter must have at least 5 points.');
        end
    ...
    if nargin >= 3 && ~isempty(x_points_inter) && length(x_points_inter) >= 5
    ...
    % Final validation
    if isempty(x_points) || length(x_points) < 5
        error('At least 5 interpolation points are required.');
    end
    ...
    x_max = max(x_points);  % Use maximum frequency selection point
    ...
    y_data_min = min([min(exact_real), min(num_real), min(exact_real_colloc)]);
    y_data_max = max([max(exact_real), max(num_real), max(exact_real_colloc)]);
