...
%   gamma_array : vector of gamma values in (0,2]
    ...
    xlabel('\\gamma');
    ylabel('L_\\infty Error (Absolute)');
    title(sprintf('Absolute L_\\infty Error vs \\gamma (M = %d)', M));
    grid on;
    set(gca, 'FontSize', 12);
    ...
    xlabel('\\gamma');
    ylabel('Relative L_\\infty Error');
    title(sprintf('Relative L_\\infty Error vs \\gamma (M = %d)', M));
    grid on;
    set(gca, 'FontSize', 12);
