function [x_opt, A, b, L2_error] = solve_redu_inter_bdy_iN1_h(gamma, h, x_points_inter, plot_flag)
% Input:
...
% Output:
%  params = [a1 a2 a3 a4 a5 a6 alpha beta] (a7 eliminated, a7 = -(a1+a2+a3+a4+a5+a6))
...
%  L2_error: L^2 error of the approximation (optional)
    ...
    % Define the basis function (cos(kx) - 1 form, mirror symmetric to i0)
    phi_r = cell(8, 1);
    phi_i = cell(8, 1);
    ...
    % Define alpha
    phi_r{7} = @(x) -  c * x.^gamma .* cos(x) - s * x.^gamma .* sin(x);
    phi_i{7} = @(x) -  s * x.^gamma .* cos(x) + c * x.^gamma .* sin(x);
    ...
    if plot_flag
        ...
        % Print L^2 error
        fprintf('Boundary iN1 scheme: L^2 error = %.6e (gamma=%.2f, h=%.6e)\n', L2_error, gamma, h);
        ...
        title(sprintf('L^2 Error - iN1: \\gamma=%.2f, h=%.6e, L^2=%.4e, Range=[0, %.2f]', ...
