function [x_opt, A, b, L2_error] = solve_redu_inter_bdy_iN_h(gamma, h, x_points_inter, plot_flag)
% Input:
...
% Output:
%  params = [a1 a2 a3 a5 a6 a7 alpha beta] (a4 eliminated, a4 = -(a1+a2+a3+a5+a6+a7))
...
%  L2_error: L^2 error of the approximation (optional)
    ...
    if isscalar(h) && h > 0 && h <= 1
        ...
        if w_max > pi/32
            ...
            x_points = [
                max(w_max / 8, pi/64),   ... Low frequency (but not too small)
    ...
    if plot_flag
        ...
        % Print L^2 error
        fprintf('Boundary iN scheme: L^2 error = %.6e (gamma=%.2f, h=%.6e)\n', L2_error, gamma, h);
        ...
        title(sprintf('L^2 Error - iN: \\gamma=%.2f, h=%.6e, L^2=%.4e, Range=[0, %.2f]', ...
