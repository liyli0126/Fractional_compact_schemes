...
%      - Can be a scalar (single test) or a vector (multiple tests)
    ...
    if single_test
        ...
        title(sprintf('Numerical Solution, \\gamma=%.2f, N=%d', gamma, M), ...
              'FontSize', 14, 'FontWeight', 'bold');
        set(gca, 'FontSize', 12);
...
function [L1_err, L2_err, Lmax_err, rel_L1, rel_L2, rel_Lmax, X, Y, u_exact_2D, u_num_2D, err_2D, use_S_form, S_operator_error] = ...
    ...
    if use_S_form
        ...
        [X, Y] = meshgrid(x, y);
        coeff1 = 1 + 2 * (2*pi*k_wave)^gamma * cos(pi*gamma/2);
        coeff2 = (2*pi*k_wave)^gamma * sin(pi*gamma/2);
        f_2D = coeff1 * sin(2*pi*X) .* sin(2*pi*Y) + coeff2 * sin(2*pi*(X + Y));
        f_vec = f_2D(:);
...
function [A_row, B_row] = build_row_internal_periodic(M, h, gamma, coeffs, j)
    ...
    B_row(idx_p1) = a5 * scale;
    B_row(idx_p2) = a6 * scale;
    B_row(idx_p3) = a7 * scale;
