function [x_opt, A, b, L2_error] = solve_redu_inter_bdy_i1_h(gamma, h, x_points_inter, plot_flag)
% Input:
...
% Output:
%  params = [a2 a3 a4 a5 a6 a7 alpha beta] (a1 eliminated, a1 = -(a2+a3+a4+a5+a6+a7))
...
%  L2_error: L^2 error of the approximation (optional)
   ...
   % phi_i{1} = @(x) 0;
    ...
    if plot_flag
        ...
        % Print L^2 error
        fprintf('Boundary i1 scheme: L^2 error = %.6e (gamma=%.2f, h=%.6e)\n', L2_error, gamma, h);
        ...
        title(sprintf('L^2 Error - i1: \\gamma=%.2f, h=%.6e, L^2=%.4e, Range=[0, %.2f]', ...
