function [x_opt, A, b, L2_error] = solve_redu_inter_bdy_i2_h(gamma, h, x_points_inter, plot_flag)
% Input:
...
%  L2_error: L^2 error of the approximation (optional)
   ...
   % phi_i{2} = @(x) 0;
    ...
    if plot_flag
        ...
        % Print L^2 error
        fprintf('Boundary i2 scheme: L^2 error = %.6e (gamma=%.2f, h=%.6e)\n', L2_error, gamma, h);
        ...
        title(sprintf('L^2 Error - i2: \\gamma=%.2f, h=%.6e, L^2=%.4e, Range=[0, %.2f]', ...
