function [x_opt, A, b, L2_error] = solve_redu_inter_h(gamma, h, x_points_inter, plot_flag)
% Input:
...
%  L2_error: L^2 error of the approximation (optional)
    ...
    if plot_flag
        ...
        plot(x_points, target_r(x_points), 'ko', 'MarkerSize', 8, 'MarkerFaceColor', 'k', 'DisplayName', 'Interpolation Points');
        hold off;
        xlabel('x (frequency)', 'FontSize', 12);
        ylabel('Real Part', 'FontSize', 12);
        title(sprintf('Real Part Comparison (\\gamma=%.2f)', gamma), 'FontSize', 12);
        legend('Location', 'best', 'FontSize', 10);
        grid on;
        
        % Plot 2: Imaginary part comparison
        figure('Position', [250, 100, 600, 400]);
        ...
        plot(x_points, target_i(x_points), 'ko', 'MarkerSize', 8, 'MarkerFaceColor', 'k', 'DisplayName', 'Interpolation Points');
        hold off;
        xlabel('x (frequency)', 'FontSize', 12);
        ylabel('Imaginary Part', 'FontSize', 12);
        title(sprintf('Imaginary Part Comparison (\\gamma=%.2f)', gamma), 'FontSize', 12);
        legend('Location', 'best', 'FontSize', 10);
        grid on;
        ...
        hold on;
        plot(x_points, zeros(size(x_points)), 'ko', 'MarkerSize', 8, 'MarkerFaceColor', 'k', 'DisplayName', 'Interpolation Points');
        hold off;
        xlabel('x (frequency)', 'FontSize', 12);
